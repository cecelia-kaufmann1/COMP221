...
public class BST {
    ...
    public void insert(int newValue) {
        if(isEmpty()){
            ...
            insertRecur(newValue, this);
        }        
    }


    /*
     * This is the recursive formula that can be plugged into the insert function to recurse over
     * the tree
     */
        protected BST insertRecur(int newValue, BST tree){
            if(tree == null){
                return new BST(newValue);
            } 
            else{
                if(newValue<nodeValue){
                    tree.setLeftChild(insertRecur(newValue, tree.leftChild));
                }
                if(newValue > nodeValue){
                    tree.setRightChild(insertRecur(newValue, tree.rightChild));
                }
                return tree;
            }
        }
